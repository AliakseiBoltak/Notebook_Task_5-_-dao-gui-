...
public class FindNoteByDateFrame extends JFrame {
	...
	public FindNoteByDateFrame(int user_id) {
		this.user_id = user_id;
		setSize(270, 180);
		setTitle("Find note by date");
	...
	private void action() {
		bFind.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (noteService.findNotesByDate(user_id, tfUser.getText()) !=null) {
						query = "select * from note where users_id=" + user_id + " and date='" + tfUser.getText() + "';";
						dispose();
